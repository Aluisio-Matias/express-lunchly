...
/* search a customer and display on a different page, and allow user to 
to click on a user and redirect to user's page*/
...
router.post("/reservation/:id/edit/", async function (req, res, next) {
  try {
    const reservation = await Reservation.get(req.params.id);
    reservation.start_at = req.body.start_at;
    reservation.num_guests = req.body.num_guests;
    reservation.notes = req.body.notes;
    await reservation.save();

    return res.redirect(`/reservation/${reservation.id}/`);
  } catch (err) {
    return next(err);
  }
});
